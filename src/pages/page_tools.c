...
static void perform_data_analysis(void) {
    ...
    res.temp_min = temp[0]; res.temp_max = temp[0];
    res.humi_min = humi[0]; res.humi_max = humi[0];
    for (int i = 1; i < data_count; i++) {
        if (temp[i] < res.temp_min) res.temp_min = temp[i];
        if (temp[i] > res.temp_max) res.temp_max = temp[i];
        if (humi[i] < res.humi_min) res.humi_min = humi[i];
        if (humi[i] > res.humi_max) res.humi_max = humi[i];
    }
    ...
    for (int i = 0; i < data_count; i++) {
        if (fabs(temp[i] - res.temp_mean) > temp_std3) temp_anom++;
        if (fabs(humi[i] - res.humi_mean) > humi_std3) humi_anom++;
    }
