...
void page_system_handle_event(event_t ev)
{
    ...
    else if (strcmp(name, "System Tools") == 0) // ✅ Tools列表页
    {
        ...
        case EV_UP:
            selected_item--;
            if (selected_item < 0)
                selected_item = PAGE_COUNT - 2; // 排除汇总页
            break;
        case EV_DOWN:
            selected_item++;
            if (selected_item > PAGE_COUNT - 2)
                selected_item = 0;
            break;
        case EV_ENTER: // ✅ 进入对应页面
            ...
            {
            case 0:
                menu_current = detail_node;
                break;
            case 1:
                menu_current = gpu_node;
                break;
            case 2:
                menu_current = npu_node;
                break;
            case 3:
                menu_current = memory_node;
                break;
            }
            break;
        case EV_BACK:
    ...
    else if (strcmp(name, "CPU Detail") == 0)
    {
        ...
        case EV_UP:
            selected_item--;
            if (selected_item < 0)
                selected_item = state.cpu_core_count - 1;
            break;
        case EV_DOWN:
            selected_item++;
            if (selected_item >= state.cpu_core_count)
                selected_item = 0;
            break;
        case EV_BACK:
            selected_item = 0;
